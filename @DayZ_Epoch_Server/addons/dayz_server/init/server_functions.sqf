...
server_cleanupGroups = {
	...
	{
		if ((count units _x == 0) && !(_x getVariable["SAR_protect",false])) then { //Edited by XsG to prevent SAR_AI Units being cleaned
			deleteGroup _x;
		};
		sleep 0.001;
	} forEach allGroups;
...
server_checkHackers = {
	...
	{
		if(vehicle _x != _x && !(vehicle _x in PVDZE_serverObjectMonitor) && (isPlayer _x)  && !((typeOf vehicle _x) in DZE_safeVehicle) && (vehicle _x getVariable ["Mission",0] != 1) && (vehicle _x getVariable ["Sarge",0] != 1) ) then { //Edited by XsG to prevent mission and Sar_AI units triggering anti-hack.
...
//Set Default Starting loadout
//XSG_startinginventory =	[];   
//XSG_startingbackpack = 	[];   
//XSG_startinginfection =  0 ;  
//XSG_startingmodel =		""; 
